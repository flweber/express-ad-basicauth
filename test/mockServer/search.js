...
module.exports = function search(server, settings) {
  ...
  server.search(baseDN, function schemaSearch(req, res, next) {
    ...
    // find a user
    if (
      /^.&.objectcategory=user.*(samaccountname|useprincipalname)/i.test(filter)
    ) {
    ...
    if (
      /^.member=cn=.*,(\s*)?ou=domain (users|admins),(\s*)?dc=domain/i.test(
        filter
      )
    ) {
      const groups = schema.getByRDN(filter.replace(/member=/g, "")).attributes
        .memberOf;
      if (groups) {
        sendGroups(groups);
      }
    ...
    if (
      /^.*objectcategory=group..cn=/i.test(filter) ||
