...
function Server(options) {
  if (options) {
    ...
    if (options.certificate || options.key) {
      if (
        !(options.certificate && options.key) ||
        (typeof options.certificate !== "string" &&
          !Buffer.isBuffer(options.certificate)) ||
        (typeof options.key !== "string" && !Buffer.isBuffer(options.key))
      ) {
        throw new TypeError(
          "options.certificate and options.key " +
  ...
  if ((options.cert || options.certificate) && options.key) {
    ...
    this.server = tls.createServer(
      options,
      connectionHandler(this, log, options)
    );
  } else {
...
Server.prototype._getHandlerChain = function _getHandlerChain(req, res) {
  if (
    req.protocolOp === Protocol.LDAP_REQ_BIND &&
